...
void movimentoIndividuale(params* input, MATRIX deltaX, VECTOR deltaF, int* indRandom, MATRIX y,int zeri){
	...
	movimentoIndividuale2(input,y,deltaX,deltaF);
}
...
void fss (params* input,int zeri){
	...
	for(it=0; it<input->iter; it++){
		movimentoIndividuale(input, deltaX, deltaF, &indRandom, y, zeri);
		minimo(deltaF, input->np, &min);
		if(min!=0)
			operatoreAlimentazione(input, pesi, deltaF, min);
		calcolaI(input, I, deltaF, deltaX);
		movimentoIstintivo(input, I);	
