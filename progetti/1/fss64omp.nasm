...
loopj:	CMP R9,R12;j<d-16; 
		JGE loopzeri
		;---CASO J<D-4
		VMOVUPD YMM0, [R11+RSI*dim]
		VMOVUPD YMM1, [R11+RSI*dim+1*p*dim]
		VMOVUPD YMM2, [R11+RSI*dim+2*p*dim]
		VMOVUPD YMM3, [R11+RSI*dim+3*p*dim]
...
nextpadd: INC R9
		...
		; ------------------------------------------------------------
endloopj:	popaq				; ripristina i registri generali
...
endforj:    popaq                ; ripristina i registri generali
        mov        rsp, rbp    ; ripristina lo Stack Pointer
        pop        rbp        ; ripristina il Base Pointer
        ret            ; torna alla funzione C chiamante
...
operatoreAlimentazione:
        ...
        VMOVAPD YMM0,[RDX+RDI*dim]
        
        VDIVPD YMM0,YMM15
        VMOVAPD YMM4,[RSI+RDI*dim]
        
        VADDPD YMM0,YMM4
        
        VMOVAPD [RSI+RDI*dim],YMM0
        ;------------------------------------------------------------
        ;Sequenza di uscita dalla funzione
        ;------------------------------------------------------------
...
forjs:	 cmp R9,R14 				;(j<d-p*(UNROLL-1))?
		jge forjsRest
		...
		vmovapd YMM0, [RDX + R10*dim]
		vmovapd YMM1, [RDX + R10*dim+p*dim]
		vmovapd YMM2, [RDX + R10*dim+2*p*dim]
		vmovapd YMM3, [RDX + R10*dim+3*p*dim]

		vmovapd YMM4, [RCX + R9*dim] 			;YMM5=I[j]
		vmovapd YMM5, [RCX + R9*dim+p*dim]
		vmovapd YMM6, [RCX + R9*dim+2*p*dim]
		vmovapd YMM7, [RCX + R9*dim+3*p*dim]
		...
		vmovapd [RDX + R10*dim],YMM0
		vmovapd [RDX + R10*dim+ p*dim],YMM1
		vmovapd [RDX + R10*dim+ 2*p*dim],YMM2
		vmovapd [RDX + R10*dim+ 3*p*dim],YMM3

		add R9, p*UNROLL 				;j=j+p*UNROLL
		jmp forjs


forjsRest:	cmp R9, RSI 					;(j<d)?
		jge endForjs
		...
		vmovapd YMM1,[RCX + R9*dim] 			;I[j]=YMM5
		...
		; Sequenza di uscita dalla funzione
		; ------------------------------------------------------------
endForjs:	popaq 		; ripristina i registri generali
		mov rsp, rbp 	; ripristina lo Stack Pointer
		pop rbp 	; ripristina il Base Pointer
		ret 		; torna alla funzione C chiamante
...
movimentoVolitivo:
		...
		vbroadcastsd  	YMM0, XMM0				; YMM0 = mol	
...
forj1:		cmp		R11, R9					; (j<d*dim-dim*p*(UNROLL-1))?
		...
		imul 		R12, RCX				; R12 = d * i		
		add 		R12, R11				; R12 = d*i + j
		
		vmovapd		YMM7, [RBX + R12]			; YMM7 = x[d*i + j*p]
		vmovapd		YMM1, [RBX + R12 + p*dim]	
		vmovapd		YMM2, [RBX + R12 + 2*p*dim]	
		vmovapd		YMM3, [RBX + R12 + 3*p*dim]	
		
		vsubpd		YMM7, [RDX + R11]			; YMM7 = x[d*i + j*p] - baricentro[j*p]
		vsubpd		YMM1, [RDX + R11 + p*dim]				
		vsubpd		YMM2, [RDX + R11 + 2*p*dim]				
		vsubpd		YMM3, [RDX + R11 + 3*p*dim]				
		
		vmulpd		YMM7, YMM7				; YMM7 = (x[d*i + j*p] - baricentro[j*p])^2
		vmulpd		YMM1, YMM1
		vmulpd		YMM2, YMM2
		vmulpd		YMM3, YMM3
		
		vaddpd 		YMM4, YMM7				; XMM4 = distanzaEuclidea + (x[d*i + j*p] - baricentro[j*p])^2
		vaddpd 		YMM4, YMM1
		vaddpd 		YMM4, YMM2
		vaddpd 		YMM4, YMM3
...
endForj1:	vhaddpd		YMM4, YMM4				
		...
		add		RSI, RCX				; EDI = indRandom + i

		vmovsd		XMM5, [R14 + RSI*dim]			; XMM5 = r[indRandom]
...
forj2:		cmp		R11, R9					; (j<d*dim-dim*p*(UNROLL-1))?
		...
		vmulpd		YMM7, YMM5				; YMM7 = r[indRandom] * (x[d*i + j*p] - baricentro[j*p])
		vmulpd		YMM1, YMM5				
		vmulpd		YMM2, YMM5				
		vmulpd		YMM3, YMM5				
		
		vmulpd		YMM7, YMM6				; YMM7 = stepvol * r[indRandom] * (x[d*i + j*p] - baricentro[j*p])
		vmulpd		YMM1, YMM6				
		vmulpd		YMM2, YMM6				
		vmulpd		YMM3, YMM6
		
		vmulpd		YMM7, YMM0				; YMM7 = molt * stepvol * r[indRandom] * (x[d*i + j*p] - baricentro[j*p])
		vmulpd		YMM1, YMM0				
		vmulpd		YMM2, YMM0				
		vmulpd		YMM3, YMM0

		vdivpd		YMM7, YMM4				; YMM7 = molt * stepvol * r[indRandom] * (x[d*i + j*p] - baricentro[j*p])/distanzaEuclidea
		vdivpd		YMM1, YMM4
		vdivpd		YMM2, YMM4
		vdivpd		YMM3, YMM4

		vaddpd		YMM7, [RBX + R12]			; YMM7 = x[d*i + j*p] + molt * stepvol * r[indRandom] * (x[d*i + j*p] - baricentro[j])/distanzaEuclidea
		vaddpd		YMM1, [RBX + R12 + p*dim]	
		vaddpd		YMM2, [RBX + R12 + 2*p*dim]	
		vaddpd		YMM3, [RBX + R12 + 3*p*dim]

		vmovapd		[RBX + R12], YMM7			; x[d*i + j*p] = YMM0
...
forj2Rest:	cmp		R11, RAX					; (j<d*dim)?
		...
		vdivpd		YMM7, YMM4				; YMM7 = molt * stepvol * r[indRandom] * (x[d*i + j*p] - baricentro[j*p])/distanzaEuclidea
		vaddpd		YMM7, [RBX + R12]			; YMM7 = x[d*i + j*p] + molt * stepvol * r[indRandom] * (x[d*i + j*p] - baricentro[j])/distanzaEuclidea
		vmovapd		[RBX + R12], YMM7			; x[d*i + j*p] = YMM7 
